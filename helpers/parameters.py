...
def flattenExtendParamsets(metaparams, baseParams, extendedParams, originalBaseParams=None,
                           originalExtendedParams=None, subnodepath=''):
    """
    ...
    for nodestring in extendedParams:
        ...
        if nodestring not in baseParams:
            ...
            if type(theBaseSubNodeObject) is DotMap:
                ...
                moreParamLists = flattenExtendParamsets(metaparams, theBaseSubNodeObject, theExtendedSubNodeObject,
                                                        originalBaseParams, originalExtendedParams, subnodepath + nodestring)
...
def crossAllParamsets(metaparams, baseParams, flattenedExtendedParams, allsimparams = []):
    # if dict is empty
    if not flattenedExtendedParams:
        ...
        # (onePath,oneList) = flattenedExtendedParams.popitem()

        # print "path: " + str(onePath)
        # print "list: " + str(oneList)

        # print "Length of flattenedExtendedParams: " + str(len(flattenedExtendedParams))
        partialSimParams = crossAllParamsets(metaparams, baseParams, flattenedExtendedParams ,[])
        # print "Length of partialSimParams: " + str(len(partialSimParams))
        # print "Length of oneList: " + str(len(oneList))

        # oneListcounter = 0
        for paramvalue in oneList:
            # oneListcounter+=1
            # print "oneListcounter="+str(oneListcounter)

            # change the given paramset at the given path
            # partialSimParamscounter = 0
            for simparams in partialSimParams:
                # partialSimParamscounter+=1
                # print "partialSimParamscounter=" + str(partialSimParamscounter)
                setParamRecursively(onePath, paramvalue, simparams)
                # print "Changed param '"+onePath+"' to "+str(paramvalue)
