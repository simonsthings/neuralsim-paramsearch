...
def make_singlerun_figures(params, paramFullPath=None):
	...
	for simparams in allsimparams:
		print "Drawing "+figtypename+" figures for simulations that have '"+ simparams.extendedparamFoldername[5:] + "' in common."
		for repetitionID in xrange(metaparams.numRepetitions):
			...
			figPath = metaparams.figures_path + 'figtype_' + figBlob + '/' + simparams.extendedparamFoldername + '/' + repfolder + '/'
			os.system('mkdir -p ' + figPath)

			#oldFigpath = metaparams.figures_path + simparams.extendedparamFoldername + '/' + repfolder + '/' + metaparams.figures_basename + '_WeightsDevelopment'

			figName = metaparams.figures_basename + '_' + figBlob + '__' + paramGroupString
			figWeightsDevelopment.savefig(figPath + figName + '.png')
			#figWeightsDevelopment.savefig(figPath + figName + '.pdf')
...
def make_singleruns_html(allsimparams, metaparams,figtypename,paramGroupString,paramFullPath): # TODO: use paramGroupString and paramFullPath to name the html file better.
	...
	for repetitionID in xrange(metaparams.numRepetitions):
		...
		for simparams in allsimparams:
			imglink = simparams.figures[figtypename][repetitionID] + '.png'
